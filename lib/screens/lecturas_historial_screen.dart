...
class LecturasHistorialScreen extends StatefulWidget {
  final String pacienteId;
  final String nombrePaciente;

  const LecturasHistorialScreen({
    super.key,
  ...
  @override
  State<LecturasHistorialScreen> createState() =>
      _LecturasHistorialScreenState();
}

class _LecturasHistorialScreenState extends State<LecturasHistorialScreen> {
  // ── Filtros ──────────────────────────────────────────────────────────────
  ...
  try {
    ...
    final file = File(
      '${dir.path}/Historial_${widget.nombrePaciente}.xlsx',
    );
    ...
    await Share.shareXFiles(
      [XFile(file.path)],
      text:
          'Historial de lecturas de ${widget.nombrePaciente}',
    );
  } catch (e) {
    if (!mounted) return;
